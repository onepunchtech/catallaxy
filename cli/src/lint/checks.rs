...
const OPERATOR_GENERATED_SECRET_SUFFIXES: &[&str] = &["-tls", "-cert", "-admission"];
...
const RUNTIME_MANAGED_RESOURCES: &[&str] = &["lab-ca-bundle"];
...
/// Navigate a serde_yaml::Value by a list of keys.
...
/// Extract container image references from a workload resource's pod spec.
...
/// Check container images against the lab's image policy.
///
/// - Warns on `:latest` tags (always)
/// - Errors on missing digests when `requireDigest` is true
/// - Warns on images from disallowed registries
pub fn check_image_pins(
    resources: &[K8sResource],
    cluster: &str,
    policy: &super::ImagePolicy,
) -> Vec<Diagnostic> {
    let mut diags = Vec::new();
    ...
    for r in resources {
        ...
        for image in &images {
            ...
            // Error on missing digest when requireDigest is true
