...
| Reading another unit | subcharts, vendored into your own release                           | `peers.<x>.<field>`, and nothing else, enforced by `checks.floe-boundary`      |
...
mkFloe {
  ...
  imports = [ ./options.nix ];      # image, replicas, domain

  requires = [ "gateway" ];         # enabling this without a gateway fails eval
  ...
  module = { config, cfg, ... }:
    let gateway = config.floes.gateway.exports; in
    {
...
**`module`** is the body, gated on `enable`. `cfg` is
`config.floes.hello-world`, already resolved.
...
**`exports`** is the typed interface. Every field needs a `default`, because
a consumer may read one while computing its own option default, and the
module system evaluates that whether or not your floe is enabled.

**`requires`** at the top level names floe _names_, not tokens. Enabling
this floe without a gateway fails evaluation with a message naming both.
