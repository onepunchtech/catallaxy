# modules/cluster/components/pki-auth.nix
#
# PKI-based authentication component — merged high-level options + IR writer.
#
# Provides passwordless Kubernetes authentication via X.509 client certificates,
# designed for YubiKey PIV slots.
#
# The CA is managed locally by `cata` (not inside the cluster).
# At cluster creation, the CA cert is injected into the API server.
# User certificates are generated by `cata pki issue` and provisioned
# to YubiKeys via `cata pki provision`.
#
# Kubernetes maps:
#   Certificate CN → username
#   Certificate O (Organization) → groups
