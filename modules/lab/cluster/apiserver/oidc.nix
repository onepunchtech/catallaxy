...
let
  inherit (lib)
    ...
    mapAttrsToList
    ;
  cfg = config.cluster.apiserver.oidc;
...
in
{
  options.cluster.apiserver.oidc = {
    ...
    issuerUrl = mkOption {
      ...
      description = ''
        ...
        Use config.floes.kanidm.exports.oauth2Clients.<name>.issuer
        or lab.clusters.<name>.floes.kanidm.exports.oauth2Clients.<name>.issuer
    ...
    out = mkOption {
      type = types.attrs;
      readOnly = true;
      description = "Computed API-server arguments derived from the settings above.";
    };
  };

  config = lib.mkMerge [
    {
      cluster.apiserver.oidc.out = {
        apiServerArgs = {
